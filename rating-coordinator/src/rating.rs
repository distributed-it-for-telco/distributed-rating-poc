...
use rating_interface::{
    ...
    Usage, RatingRequest, RatingRecord, Agent,
};
...
pub async fn handle_rating_cycle(
    ...
    dfs_recursive(&_ctx,agents_graph.get_start_vertex().to_owned().unwrap().clone(), &visited,&agents_graph,&mut rating_process_request.rating_request.clone()).await
}
...
#[async_recursion]
async fn  dfs_recursive(_ctx: &Context,vertex: Agent, visited: &HashMap<String, bool>,agents_graph:&AgentGraph,rating_request: &mut RatingRequest)-> RpcResult<RatingResponse> {
    ...
    if let Some(neighbors) = agents_graph.adjacency_list.get(&vertex.identifiation.name) {
        ...
        for neighbor in neighbors {
            ...
            if !visited[&neighbor.identifiation.name] {
                  let child_rating = dfs_recursive(&_ctx,neighbor.clone(), visited,&agents_graph,rating_request).await?;
