...
impl RatingAgent for PrepaidOrangeVodOneshotAgentActor {
    ...
    async fn rate_usage(&self, _ctx: &Context, _arg: &RatingRequest) -> RpcResult<RatingResponse> {
        ...
        let balance_key = format!("{}:{}:{}", BALANCE_KEY_PREFIX, &_arg.customer_id.as_str(), OFFER_ID);
        ...
        info!("Usage {} , Rating {} , & balance {}",usage,rating,balance.balance_characteristic.count);
          if balance.balance_characteristic.count < rating {
            ...
            billing_info.messages.push(
                format!(" {} will be deducted from your balance",rating.to_string())
            );
            let new_balance = get_party_balance(_ctx, balance_key.as_str()).await?;
            info!("Usage {} , Rating {} , & balance {}",usage,rating,new_balance.balance_characteristic.count);
            billing_info.messages.push(
                format!(" Your Balance now is {} {}",new_balance.balance_characteristic.count,new_balance.balance_characteristic.unit)
            );
          }
        }
...
async fn get_party_balance(
    ...
    let balance: Balance = serde_json::from_str(&balance_json_str).map_err(|err| RpcError::Ser(format!("{}", err)))?;
...
async fn decrease_balance(
    ...
    let mut balance: Balance = serde_json::from_str(&balance_json_str).map_err(|err| RpcError::Ser(format!("{}", err)))?;

    balance.balance_characteristic.count -= rate;

    let serialized_balance = serde_json::to_string(&balance).map_err(|err| RpcError::Ser(format!("{}", err)))?;
