use rating_interface::{
    AgentList, Balance, BalanceAccessManager, GetChildrenRequest, RatingAgent, RatingAgentReceiver,
    RatingRequest, RatingResponse, RatingResponseBuilder, ValidationRequest, ValidationResponse,
};
...
impl RatingAgent for PrepaidOrangeVodOneshotAgentActor {
    async fn rate_usage(&self, _ctx: &Context, _arg: &RatingRequest) -> RpcResult<RatingResponse> {
        ...
        let balance = balance_access_manager
            .get_balance(_ctx, customer_id, OFFER_ID)
            .await?;
        ...
        if has_sufficient_balance(balance.balance_characteristic.count, rating) {
            ...
            let new_balance: Balance = balance_access_manager
                .withdraw(_ctx, customer_id, OFFER_ID, rating)
                .await?;
